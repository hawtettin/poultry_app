# bootstrap.py
# Genereaza un MVP Django+DRF pentru ferma avicola (roluri, loturi, mortalitati, tratamente, financiar, raport sezon)
...
class IsAdmin(BasePermission):
    def has_permission(self, request, view):
        return request.user.is_authenticated and (request.user.is_superuser or in_group(request.user, "ADMIN"))
...
class IsEmployeeOrAbove(BasePermission):
    def has_permission(self, request, view):
        u = request.user
        return u.is_authenticated and (u.is_superuser or in_group(u, "ADMIN") or in_group(u, "MANAGER") or in_group(u, "EMPLOYEE"))
""")
...
class MortalityEvent(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.flock_id} {self.date} -{self.count}"
...
class Treatment(models.Model):
    ...
    def save(self, *args, **kwargs):
        # calculeaza automat data de finalizare a perioadei de asteptare
        if self.end_date and self.withdrawal_days is not None:
            self.withdrawal_end_date = self.end_date + timedelta(days=int(self.withdrawal_days))
        super().save(*args, **kwargs)

    def __str__(self) -> str:
        return f"{self.flock_id} {self.product_name} ({self.start_date}->{self.end_date})"
""")
...
class TreatmentAdmin(admin.ModelAdmin):
    ...
    search_fields = ("product_name", "active_substance", "notes")
""")
...
class WithdrawalAlertsView(APIView):
    ...
    def get(self, request):
        ...
        qs = Treatment.objects.select_related("flock").filter(withdrawal_end_date__gte=today).order_by("withdrawal_end_date")
        data = TreatmentSerializer(qs, many=True).data
        return Response({"today": str(today), "active_withdrawals": data})
""")
...
class Document(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.doc_type} {self.doc_no} ({self.date})"
...
class DocumentLine(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(*args, **kwargs)
        # dupa salvare, recalculam totaluri document
...
from django.contrib import admin
from .models import Partner, Category, Document, DocumentLine, Payment, Attachment
...
class DocumentAdmin(admin.ModelAdmin):
    ...
    search_fields = ("doc_no", "partner__name")
...
class AttachmentAdmin(admin.ModelAdmin):
    list_display = ("id", "document", "original_name", "uploaded_by", "uploaded_at")
""")
...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["subtotal", "total", "created_by", "created_at", "approved_by", "approved_at"]
    ...
    def update(self, instance, validated_data):
        # simplu: update campuri + optional inlocuire linii (daca vin)
...
def write_all():
    ...
    print("Urmatorii pasi (Windows):")
    print("  1) Copiaza .env.example -> .env si editeaza daca vrei")
    print("  2) python -m venv .venv")
    print("  3) Activeaza venv (CMD): .venv\\Scripts\\activate.bat")
    print("  4) pip install -r requirements.txt")
    print("  5) docker compose up -d")
    print("  6) python manage.py makemigrations")
    print("  7) python manage.py migrate")
    print("  8) python manage.py init_roles")
...
def main() -> None:
    ...
    print("✅ Proiect generat in:", BASE)
    print("➡️ Continua cu pasii de mai jos.")
